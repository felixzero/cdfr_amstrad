...
uint8_t sprite_drawing_order[MAX_NUMBER_OF_SPRITES];
...
void clear_sprites(void)
{
    ...
    for (sprite_handle_t i = 0; i < number_of_sprites; ++i) {
        sprite_handle_t j = sprite_drawing_order[i];
...
void recalculate_drawing_order(void)
{
    for (uint8_t i = 0; i < number_of_sprites; ++i) {
        sprite_drawing_order[i] = i;
    }

    for (uint8_t i = 0; i < number_of_sprites; ++i) {
        uint8_t x = sprite_drawing_order[i];
        uint8_t j = i;
        while (j > 0 && sprite_info_table[sprite_drawing_order[j - 1]].z_index > sprite_info_table[x].z_index) {
            sprite_drawing_order[j] = sprite_drawing_order[j - 1];
            j--;
        }
        sprite_drawing_order[j] = x;
    }
}
